  ...
  rules: {
    ...
    if_statement_definition: ($) =>
      seq(
        ...
        choice(seq($.field_declaration_list), seq(":", $.component_definition)),
      ),
    ...
    property_type_list: ($) => seq("[", field("type", $._type_identifier), "]"),

    list_definition: ($) => seq($.list_definition_body, "]"),

    list_definition_body: ($) =>
      seq("[", repeat(seq($.struct_field_declaration_list, optional(",")))),
    ...
    comparison_binary_expression: ($) =>
      prec.left(0, seq($._expression, $.comparison_operator, $._expression)),

    mult_binary_expression: ($) =>
      prec.left(2, seq($._expression, $.mult_prec_operator, $._expression)),
    ternary_expression: ($) =>
      prec.left(3, seq($._expression, "?", $._expression, ":", $._expression)),

    add_binary_expression: ($) =>
      prec.left(1, seq($._expression, $.add_prec_operator, $._expression)),
    ...
    //FIXME: Having the \d on the end is a hack that works for most cases but needs to be fixed
    _identifier: ($) => /([a-zA-Z_]+-?)+\d?/,
